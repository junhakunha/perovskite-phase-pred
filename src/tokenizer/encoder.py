...
class EncoderNetwork(nn.Module):
    ...
    def forward(self, x: tuple[Tensor, tuple[str]]) -> Tensor:
        ...
        src_embedded, src_key_pad_mask = self.src_fwd_fn(
            src, self.d_model, self.src_embed, self.src_pad_token, self.pos_encoder)
        if self.is_causal:
            src_mask = torch.nn.Transformer.generate_square_subsequent_mask(
                src_embedded.size(1)).to(src_embedded.device)
        else:
            src_mask = None
    ...
    def save_embeddings(self,
        ...
        src_out = self.encoder(src=src_embedded,
                               ...
                               src_key_padding_mask=src_key_pad_mask,
                               is_causal=self.is_causal)
        all_embeddings = src_out[:, -1, :]
        all_smiles_to_embeddings = dict(zip(all_smiles, all_embeddings))
        torch.save(all_smiles_to_embeddings, "all_smiles_to_embeddings.pt")
